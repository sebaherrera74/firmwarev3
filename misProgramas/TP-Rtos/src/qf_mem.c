...
*
* @note
* This function can be called from any ISR level.
*
* @sa
* QMPool_get()
*
...
* @include qmp_use.c
*/
void QMPool_put_fromISR( QMPool * const me, void *b)
{
    UBaseType_t uxSavedInterruptStatus;
    /** @pre # free blocks cannot exceed the total # blocks and
    * the block pointer must be from this pool.
...
/**
* @description
* The function allocates a memory block from the pool inside of ISR and returns a pointer
* to the block back to the caller.
*
* @param[in,out] me      pointer (see @ref oop)
* @param[in]     margin  the minimum number of unused blocks still available
...
* @include qmp_use.c
*/
void *QMPool_get_fromISR( QMPool * const me, unsigned short const margin )
{
	UBaseType_t uxSavedInterruptStatus;
	QFreeBlock *fb;
